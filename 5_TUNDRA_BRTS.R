...
# R.utils PACKAGES TO COMPLETE THIS ANALYSIS.                                       
#                                                                                   
# FILE REQUIREMENTS:
#   (1) Spatial data to train and test BRT analysis:
...
#         BRTs.
# DEPENDENCIES:
#   (1) aucroc.R - CUSTOM WRITTEN FUNCTION THAT DOES THE AUC ANALYSIS
#   (2) gbm package (version 2.1.1)
...
# SET MAIN WORKING DIRECTORY. SET THIS TO BE THE LOCATION WHERE THE DIRECTORY:
# 'Young_et_al_2016_Ecography', AND ALL ITS ASSOCIATED SUB-DIRECTORIES, IS LOCATED. 
wdir <- "G:/test"
# DIRECTORY TO SAVE RESULTS TO 
save_dir <- "/Young_et_al_2016_Ecography/Output/3_TUNDRA_FINAL_RESULTS/"
# CREATE DIRECTORY, IF IT DOES NOT ALREADY EXIST, TO STORE RESULTS
direxist <- dir.exists(paste(wdir,save_dir,sep = ""))
if (direxist == FALSE){
  dir.create(paste(wdir,save_dir,sep = ""),
             recursive = TRUE)
}
...
source(paste(wdir,"/Young_et_al_2016_Ecography/Scripts/Functions/aucroc.R",
...
setwd(paste(wdir,"/Young_et_al_2016_Ecography/Data/VegLandscapeData",
            sep = ""))
...
ecorinfo <- read.csv(paste(wdir,"/Young_et_al_2016_Ecography/Data/AncillaryData/ecor_info.csv",
                           sep = ""),
                     header = FALSE)
...
for (i in 1:nmodels){
  ...
  setwd(paste(wdir,"/Young_et_al_2016_Ecography/Data/train_test_data",sep = ""))
  ...
  # THIS USES gbm VERSTINO 2.1.1 RELEASED ON 3-11-2015. META-PARAMETER VALUES FOR 
  ...
  model_valid_err[,i] <- brt_i$valid.error
  ...
  writeRaster(pred_map,
              paste(wdir,save_dir,"pred_map_",i,".tif",sep = ""),
              format = "GTiff",
              overwrite = TRUE)
  
  # OUTPUT BRT AS A RDATA FILE
  save(brt_i,file = paste(wdir,save_dir,"brt_",i,".RData",sep = ""))
...
setwd(paste(wdir,save_dir,sep = "")) # CHANGE DIRECTORY TO WHERE THE RESULTS SHOULD
                                     # BE STORED

...
#       TempWarm & AnnDEF. THESE DATA ARE IN A GEOTIFF FORMAT. LOCATED IN:
#       '\Young_et_al_2016_Ecography\Data\FutureClimatologies\'
...
#       THESE FILES ARE LOCATED IN: \Young_et_al_2016_Ecography\Output\1_AK_FINAL_RESULTS\
#       THESE FILES ARE IN AN RDATA FORMAT.
#
# DEPENDENCIES:
#   (1) gbm package (version 2.1.1)
...
wdir <- "G:/test"
save_dir <- "/Young_et_al_2016_Ecography/Output/4_FUTURE_PROJECTIONS/"
...
# LOAD STATIC (I.E., UNCHANGING) VEGETATION AND TR DATA TO DRIVE BRTS
...
for (g in 1:length(gcms)){ # FOR EACH GCM ...
  ...
  if (file.exists(paste(wdir,save_dir,gcms[g],sep = "")) == FALSE){
    mkdirs(paste(wdir,save_dir,gcms[g],sep = ""))
  }
  
  for (y in 1:length(yrs)){ # FOR EACH OF THE THREE TIME PERIODS ...
    
    # CHANGE WORKING DIRECTORY
    setwd(paste(wdir,"/Young_et_al_2016_Ecography/Data/FutureClimatologies/",gcms[g],
                sep = ""))
    ...
    setwd(paste(wdir,"/Young_et_al_2016_Ecography/Output/1_AK_FINAL_RESULTS",sep = ""))
    ...
    foreach(i = 1:100,.packages=c("raster","gbm")) %dopar% {
      ...
      writeRaster(pred_map,
                  paste(wdir,save_dir,gcms[g],"/",gcms[g],
                        "_pred_map_",yrs[y],"_",i,".tif",sep=""),
                  format="GTiff",
                  overwrite=TRUE)

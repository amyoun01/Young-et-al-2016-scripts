...
% Maps are displayed using an Albers equal-area projection. 
%
% FILE REQUIREMENTS:
...
%    using the script '8_corr_results.m'
%
% DEPENDENCIES:
...
output_dir = [wdir,'\Young_et_al_2016_Ecography\Output\'];
...
cd([wdir,'\Young_et_al_2016_Ecography\Data\AncillaryData\Shapefiles']);
...
cd([output_dir,'1_AK_FINAL_RESULTS']); 
ak_hist = importdata('med_pred_prob.mat');
cd([output_dir,'2_BOREAL_FINAL_RESULTS']); 
bor_hist = importdata('med_pred_prob.mat');
cd([output_dir,'3_TUNDRA_FINAL_RESULTS']); 
...
save_dir = [wdir,'\Young_et_al_2016_Ecography\Output\5_FIGS'];
...
for i = 1:length(indfolders) % FOR EACH MODEL
    ...
    % CHANGE DIRECTORY
    directory = [output_dir,char(indfolders(i))];
    cd(directory);
